...
class PWApi
{
    ...
    public function __construct()
    {
        ...
        $this->data = Structure::getStructure('101');
    }
...
return true;*/
     ...
     * @return boolean
     */
    public function sendMail($receiver, $title, $context, $item = [], $money = 0)
    {
        if ($item === []) {
        ...
        while (true) {
            try {
                if ($this->online) {
                    ...
                    if (isset($data['RoleList']['users']) && is_array($data['RoleList']['users'])) {
                        foreach ($data['RoleList']['users'] as $user) {
                        ...
                        // Break the loop if online users are found
                        if (!empty($online)) {
                            break;
                        }
                    }
...
return $this->gamed->unmarshal($data, config('FactionFortressDetail'));*/
    ...
    {
        return @fsockopen(Config::get('local', '127.0.0.1'), Config::get('ports.client'), $errCode, $errStr, 1) ? true : false;
    }
    public function ports()
    {
        $ports = [];
        $port_list = Config::get('ports.client');
        foreach ($port_list as $name => $port) {
            $ports[$name]['port'] = $port;
            $ports[$name]['open'] = @fsockopen(Config::get('local', '127.0.0.1'), $port, $errCode, $errStr, 1) ? true : false;
        }
        return $ports;
    }
}