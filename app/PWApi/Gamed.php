...
class Gamed
{
    ...
    {
        return $this->SendToSocket($data, Config::get('ports.gamedbd'));
    }
    public function SendToDelivery($data)
    {
        return $this->SendToSocket($data, Config::get('ports.gdeliveryd'), true);
    }
    public function SendToProvider($data)
    {
        return $this->SendToSocket($data, Config::get('ports.gprovider'));
    }
    public function SendToSocket($data, $port, $RecvAfterSend = false, $buf = null)
    {
        if (@fsockopen(Config::get('local', '127.0.0.1'), $port, $errCode, $errStr, 1)) {
            $sock = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
            socket_connect($sock, Config::get('local', '127.0.0.1'), $port);
            if (Config::get('s_block', false)) {
            ...
            switch (Config::get('s_readtype', 3)) {
                case 1:
                    socket_recv($sock, $buf, Config::get('maxbuffer', 65536), 0);
                    break;
                case 2:
